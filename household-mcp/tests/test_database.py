...
def test_existing_rf04_database_upgrades_to_monitoring_without_losing_data(tmp_path) -> None:
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        connection.execute(
            ...
            INSERT INTO savings_goals (
                id, name, target_amount_idr, status, created_at, updated_at
            ) VALUES (?, 'Synthetic goal', 1000000, 'ACTIVE', ?, ?)
            """,
            (str(uuid4()), "2099-01-01T00:00:00Z", "2099-01-01T00:00:00Z"),
        )
    ...
    with database.connect() as connection:
        ...
        goals = connection.execute("SELECT COUNT(*) FROM savings_goals").fetchone()[0]
